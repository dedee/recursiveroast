...
public class WopperModel {
	...
	private List<List<CmdReplacement>> recursions;
	private List<Cmd> constants;

	public WopperModel() {
		Commands.getInstance().reset();
		recursions = new ArrayList<List<CmdReplacement>>();
		constants = new ArrayList<Cmd>();
	}
	...
	private void addConstant(String name, String value) throws IOException {
		name = name.trim();
		value = value.trim();
		if (name.equals("rektiefe") || name.equals("recursion")) {
			recursionDepth = Integer.parseInt(value);
			System.out.println("rec depth: " + recursionDepth);
		} else if (name.equals("winkel") || name.equals("angle")) {
			angle = Double.parseDouble(value);
			System.out.println("angle: " + angle);
		} else if (name.equals("initialangle")) {
	...
	private void addRule(int recursionDepth, char name, String value)
		...
		if (listOfRecursion == null) {
			listOfRecursion = new ArrayList<CmdReplacement>();
	...
	public String getLSystemAsString() {
		...
		sb.append("angle = " + getAngle() + "\n");
		sb.append("initialangle = " + getInitialAngle() + "\n");
		sb.append("recursion = " + getRecursionDepth() + "\n");
		...
		for (int i = 0; i < recursions.size(); i++) {
			...
			if (list != null) {
				for (CmdReplacement cpr : list) {
					sb.append(i + ": "
							+ Commands.getInstance().idToChar(cpr.what));
					sb.append(" = ");
					if (cpr.with != null) {
